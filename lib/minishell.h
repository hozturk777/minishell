...
/*   Updated: 2025/10/05 20:52:59 by hasivaci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_command				*parse_single_command(t_list **token_node,
							t_global *global);
...
void					append_command_to_chain(t_command *head,
							t_command *new_cmd);
void					add_redirect_to_command(t_command *cmd,
							t_redirect *redirect);

char					*handle_dollar_expansion(char *input, int *i,
							t_global *global);
...
int						execute_pipeline_command(t_command *cmd,
							t_global *global, int prev_fd, int *pipe_fd);
pid_t					execute_pipeline_command_async(t_command *cmd,
							t_global *global, int prev_fd, int *pipe_fd);
...
void					setup_pipeline_fds(t_command *cmd, int prev_fd,
							int *pipe_fd);
...
void					execute_redirect_command(t_command *cmd,
							t_global *global, int *origin);
...
void					check_path(t_command *cmd, t_list *node,
							int *originals);
...
int						is_equal_export(char *equal_pos, char *merged_arg,
							t_global *global);
void					expand_double_quotes(t_command *cmd, t_global *global,
							int *i);
void					expand_single_quotes(t_command *cmd, int *i);
void					need_expansion_utils(t_command *cmd, t_global *global,
							int *i);
